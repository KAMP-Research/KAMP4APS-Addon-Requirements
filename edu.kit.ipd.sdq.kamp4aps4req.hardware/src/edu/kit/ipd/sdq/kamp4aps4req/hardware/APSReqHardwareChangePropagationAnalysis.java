...
import edu.kit.ipd.sdq.kamp4aps.model.KAMP4aPSModificationmarks.impl.KAMP4aPSModificationRepositoryImpl;
...
public class APSReqHardwareChangePropagationAnalysis extends AbstractAPSReqChangePropagationAnalysis<APSReqHardwareArchitectureVersion> {
	...
	public void runChangePropagationAnalysis(APSReqHardwareArchitectureVersion version) {
		...
		// add new APS seed modifications generated by previous step
		this.addAPSSeedModifications(version);
	...
	*/
	...
	private void addAPSSeedModifications(APSReqHardwareArchitectureVersion version) {
		version.getModificationMarkRepository().getSeedModifications().getModuleModifications().clear();
		version.getModificationMarkRepository().getSeedModifications().getInterfaceModifications().clear();
		version.getModificationMarkRepository().getSeedModifications().getStructureModifications().clear();
		version.getModificationMarkRepository().getSeedModifications().getComponentModifications().clear();
		APSReqHardwareChangePropagationDueToSpecificationDependencies hardwareChanges = 
				(APSReqHardwareChangePropagationDueToSpecificationDependencies) this.getChangePropagationDueToSpecificationDependencies();
		
		for (APSReqModifyEntity modification : hardwareChanges.getEntityModifications()) {
			Entity affected = modification.getAffectedElement();
			
			if (affected != null) {
				if (affected instanceof Structure) {
					ModifyStructure<Structure> mod = KAMP4aPSModificationmarksFactory.eINSTANCE.createModifyStructure();
					mod.setAffectedElement((Structure)affected);
					version.getModificationMarkRepository().getSeedModifications().getStructureModifications().add(mod);
				} else if (affected instanceof Module) {
					ModifyModule<Module> mod = KAMP4aPSModificationmarksFactory.eINSTANCE.createModifyModule();
					mod.setAffectedElement((Module)affected);
					version.getModificationMarkRepository().getSeedModifications().getModuleModifications().add(mod);
				} else if (affected instanceof Interface) {
					ModifyInterface<Interface> mod = KAMP4aPSModificationmarksFactory.eINSTANCE.createModifyInterface();
					mod.setAffectedElement((Interface)affected);
					version.getModificationMarkRepository().getSeedModifications().getInterfaceModifications().add(mod);
				} else if (affected instanceof Component) {
					ModifyComponent<Component> mod = KAMP4aPSModificationmarksFactory.eINSTANCE.createModifyComponent();
					mod.setAffectedElement((Component)affected);
					version.getModificationMarkRepository().getSeedModifications().getComponentModifications().add(mod);
				}
			}
